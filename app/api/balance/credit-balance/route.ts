...
async function creditLeaveBalance(
  ...
  for (const leaveType of leaveTypes) {
    ...
    if (totalBalances.length === 0 || !existingBalance) {
      ...
      // existingBalance exists → accrue toward the policy cap (if any)
      const cap = leaveType.capped; // e.g. Earned Leave = 18, from POLICIES

      // Only credit if uncapped, or still below the cap
      if (cap === undefined || existingBalance.balance < cap) {
        const rawBalance = existingBalance.balance + leaveType.accural;
        // Clamp the RESULT so it can never exceed the cap (17.5 + 1 → 18, not 18.5)
        const newBalance =
          cap === undefined ? rawBalance : Math.min(cap, rawBalance);
    ...
    await prisma.employeeLatestIncrement.upsert({
      where: {
        employee_id_policyName: {
